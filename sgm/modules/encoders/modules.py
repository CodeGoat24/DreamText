...
class GeneralConditioner(nn.Module):
    ...
    def __init__(self, emb_models: Union[List, ListConfig]):
        ...
        for n, embconfig in enumerate(emb_models):
            if embconfig.get("is_trainable", False):
                embconfig['params']['trainable'] = embconfig.get("is_trainable", False)
        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    if hasattr(embedder, "trainable"):
                        emb_out, clip_loss = embedder(batch[embedder.input_key], batch['bbox_region'])
                        output["clip_loss"] = clip_loss
                    else:
...
class LabelEncoder(AbstractEmbModel, pl.LightningModule):
    ...
    def __init__(self, max_len, emb_dim, n_heads=8, n_trans_layers=12, ckpt_path=None, trainable=False,
                 lr=1e-4, lambda_cls=0.01, lambda_pos=0.01, clip_dim=1024, visual_len=197, visual_dim=768, visual_config=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def tune(self):
        modules = []
        modules.append(self.visual_head)
        for module in self.encoder._modules['layers'][-2:]:
    ...
    def forward(self, labels, bbox_image=None):
        ...
        if bbox_image is None:
            ...
            clip_loss, cls_loss, pos_loss = self.get_loss(text_out, visual_out, clip_target, cls_out, pos_out, cls_target, pos_target)

            loss = clip_loss + self.lambda_cls * cls_loss + self.lambda_pos * pos_loss

            return text_emb, loss
        return text_emb
    ...
    def get_loss(self, text_out, visual_out, clip_target, cls_out, pos_out, cls_target, pos_target):
        ...
        return clip_loss, cls_loss, pos_loss
